...
public class JwtTokenUtil implements Serializable {
    ...
    private String doGenerateToken(Map<String, Object> claims, String subject) {
        ...
        //下面就是在为payload添加各种标准声明和私有声明了,标准中注册的声明 (建议但不强制使用),
        return Jwts.builder() //这里其实就是new一个JwtBuilder，设置jwt的body
                .setClaims(claims)  // payload 载体信息中私有声明，如果有私有声明，一定要先设置这个自己创建的私有的声明，这个是给builder的claim赋值，一旦写在标准的声明赋值之后，就是覆盖了那些标准的声明的
                .setSubject(subject) //sub(Subject)：代表这个JWT的主体，即它的所有人，这个是一个json格式的字符串，可以存放什么userid，roldid之类的，作为什么用户的唯一标志。
                .setIssuedAt(createdDate)  //jwt的签发时间
                .setExpiration(expirationDate) // jwt的过期时间，这个过期时间必须要大于签发时间
